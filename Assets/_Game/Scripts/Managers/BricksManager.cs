...
public class BricksManager : MonoBehaviour
{
    ...
    private void GenerateBricks()
    {
        ...
        int[,] curentLevelData = this.LevelsData[this.CurrentLevel];
        ...
        for(int row = 0; row < this.maxRows; row++)
        {
            ...
            {
                int brickType = curentLevelData[row, col];

                if(brickType > 0)
                {
                    Brick newBrick = Instantiate(brickPrefab, new Vector3(currentSpawnX, currentSpawnY, 0.0f - zShift),
                        Quaternion.identity) as Brick;

                    newBrick.Init(bricksContainer.transform, this.Sprites[brickType - 1], this.BricksColors[brickType], brickType);
    ...
    {
        TextAsset text = Resources.Load("levels.txt") as TextAsset;
